...
**Race courses (v1.3.2):** the server's `generateCourse(seed, n)` lands every course a seeded random 3–15 km from the home runway (like the battle arena) and describes gates as `{ x, z, alt, r }` — `alt` is height *above the surface*, because the server knows nothing about the terrain. `RaceManager._placeCourse` turns that into world positions on every client: a gate sits `alt` above the highest surface (ground, or water level over the sea) probed every `RACE_GATE_PROBE_STEP` m along the legs into and out of it, so the straight line between consecutive gates always clears the ground; terrain is deterministic, so all clients agree. 8 gates = one ring lap; 16/32 = a wandering course (each leg turns 15–70°, alternating-biased, bent back toward the centre when it strays, min gate spacing enforced). The start line faces along the first leg (`_prevOf`).
